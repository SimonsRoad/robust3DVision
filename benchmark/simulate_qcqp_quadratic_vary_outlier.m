...
NUM_ITERATIONS = 20; % run SDP multiple times
NOISE_LEVEL = 1; % in pixel
FIELD_OF_VIEW = 100; % in degress
NUM_POINTS = 100;
MAX_PARALLAX = 2; % in meters
OUTLIER_RATIO = [0.0, 0.01, 0.05, 0.1, 0.2];
...
for ratio_itr=1:length(OUTLIER_RATIO)
    outlier_ratio = OUTLIER_RATIO(ratio_itr);
    ...
    performance{ratio_itr,1} = finalErrorQCQP;
    performance{ratio_itr,2} = finalTightness;
end
