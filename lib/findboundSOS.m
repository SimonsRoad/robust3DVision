function [GAM,vars,xopt,prog] = findboundSOS(p,ineq,eq,DEG,options)
...
if (info.dinf>1e-2)|(info.pinf>1e-2)
    ...
    epsilon = 1e-3;
    ix = find(abs(sum(prog.extravar.Z{1},2)-1) < epsilon);
...
%     varsNum(1) = double(sosgetsol(prog,t1,16));
%     varsNum(2) = double(sosgetsol(prog,t2,16));
%     varsNum(3) = double(sosgetsol(prog,t3,16));
%     varsNum(4) = double(sosgetsol(prog,R1_1,16));
%     varsNum(5) = double(sosgetsol(prog,R1_2,16));
%     varsNum(6) = double(sosgetsol(prog,R1_3,16));
    
    % If the upper and lower bounds are close (absolute or relative), return them
    ach = double(subs(p,num2cell(vars).',num2cell(xopt).'));
    tolerance = 1e-3;
    if min(abs(ach/GAM-1),abs(ach-GAM)) > tolerance ; 
        xopt = [];
        return;
    end
