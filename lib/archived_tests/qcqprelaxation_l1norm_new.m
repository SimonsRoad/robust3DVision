function [RStar,tStar,recoverStatus,cvxStatus,pStarRelax,tightness,WStar] = ...
    qcqprelaxation_l1norm_new(bearing1,bearing2,quiet)
...
DIM_w_HOMO = DIM_z_HOMO + NUM_POINTS; % including binary variables
...
for i=1:NUM_POINTS
    ...
    b_c(1,1) = -1;
    b_c(DIM_z_HOMO+i, DIM_z_HOMO+i) = 1;
    Q_b{i} = b_c;
end
...
for i=1:NUM_POINTS
    ...
    abs_c(DIM_z_HOMO+i, xIndex) = A/2;
    abs_c(xIndex, DIM_z_HOMO+i) = A'/2;
    Q_abs{i} = abs_c;
end
...
%% form constraints on two ways of expressing gi(x)
tIndex = [11, 21, 31];
rIndex = 2:10;
Q_tr = {};
for i=1:NUM_POINTS
    ...
    A = reshape(C', [numel(C), 1]);
    A = A';
    tr_1 = zeros(DIM_w_HOMO, DIM_w_HOMO);
    tr_1(1, xIndex) = A/2;
    tr_1(xIndex, 1) = A'/2;
    tr_2 = zeros(DIM_w_HOMO, DIM_w_HOMO);
    tr_2(tIndex, rIndex) = C/2;
    tr_2(rIndex, tIndex) = C'/2;
    Q_tr{i} = tr_1 - tr_2;
end

%% form cost function cost = b1*y1+b2*y2+...+bN*yN
Q_0 = zeros(DIM_w_HOMO, DIM_w_HOMO);
for i=1:length(Q_abs)
    Q_0 = Q_0 + Q_abs{i};
end
...
WStar = WStar(1:DIM_z_HOMO, 1:DIM_z_HOMO);
